							...
							<?php if(isset($slider_list) && count($slider_list)>0){ ?>
									...
									<?php foreach($slider_list as $list){ ?>
										<tr>
											...
											<td class="text-right">
												<a href="<?php echo base_url('slider/status/'.base64_encode($list['s_id']).'/'.base64_encode($list['status'])); ?>" class="btn btn-danger"><?php if($list['status']==0){ echo "Active"; }else{ echo "Deactive";} ?></a>
